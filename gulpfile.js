...
gulp.task('es6', ['clean', 'lint'],  function () {
    ...
    return gulp.src('src/*.js')
    	.pipe($.flatten() )
		.pipe( $.rename(function(path){ path.basename = 'element-state-manager.' + path.basename; }) )
...
gulp.task('es5', ['clean', 'lint'],  function () {
    ...
    return gulp.src('src/build.js')
        ...
        .pipe( $.babel() )
    	.pipe($.flatten())
		.pipe( $.rename(function(path){ path.basename = path.basename.replace('build', 'element-state-manager'); }) )
